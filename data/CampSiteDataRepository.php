...
class CampSiteDataRepository implements DataRepository
{
    ...
    {
        $query = "UPDATE CampSite SET name= ? location = ?, price = ?, description = ?,local_attraction=?, features = ?, map_iframe=?, notice_note = ?,pitch_type_id = ? WHERE site_id = ?;";
        $paramTypes = "ssissssii";
        $stmt = prepareAndExecuteQuery($this->connection, $query, $paramTypes, $newData->getName(), $newData->getLocation(), $newData->getPrice(), $newData->getDescription(), $newData->getLocalAttraction(), $newData->getFeatures(),$newData->getMapIframe(), $newData->getNoticeNote(), $newData->getPitchTypeId(), $existingData->getSiteId());
    ...
    {
        $query = "INSERT INTO CampSite (name,location, price, description,local_attraction,features,map_iframe,notice_note,pitch_type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
        $paramTypes = "ssisssssi";
        $stmt = prepareAndExecuteQuery($this->connection, $query, $paramTypes, $data->getName(), $data->getLocation(), $data->getPrice(), $data->getDescription(), $data->getLocalAttraction(), $data->getFeatures(), $data->getMapIframe(),$data->getNoticeNote(), $data->getPitchType()->getPitchTypeId());
    ...
    private function mapRowToCampSiteObject($row): CampSite
    {
        ...
        $campsite->setMapIframe($row["map_iframe"]);
        return $campsite;
    }
